...
freq_itemsets = apriori(basket, min_support=0.001, use_colnames=True, max_len=4, low_memory=True)
freq_itemsets = freq_itemsets.sort_values(["itemsets", "support"]).reset_index(drop=True)
...
def build_emoji_network(basket, freq_itemsets, top_n=50, min_pair_support=0.0005):
    ...
    one_item = freq_itemsets[freq_itemsets["itemsets"].apply(lambda s: len(s)==1)].copy()
    if one_item.empty:
        supp = basket.mean(axis=0).to_dict()
    else:
        supp = {next(iter(s)): v for s, v in zip(one_item["itemsets"], one_item["support"])}
    ...
    two_item = freq_itemsets[freq_itemsets["itemsets"].apply(lambda s: len(s)==2)].copy()
    edges = []
